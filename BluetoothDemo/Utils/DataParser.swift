...
class DataParser {
    ...
    var pointArray: [CGPoint]{
        get {
            var array = [CGPoint]()
            queue.sync { array = self.waveArray }
            return array
        }
    }
...
extension DataParser{
    ...
    func startTimer(){
        recordTimer = Timer.publish(every: 1, on: RunLoop.main, in: RunLoop.Mode.default).autoconnect()
            .receive(on: queue)
        ...
        waveTimer = Timer.publish(every: 0.033, on: RunLoop.main, in: RunLoop.Mode.default).autoconnect()
            .receive(on: queue)
            .sink(receiveValue: { (_) in
